...
function prompt_git() {
  prompt_getcolors
  local status output flags ansi brief
  ansi="39;42"
  ...
  brief="$(git status -sb 2>/dev/null)"
  if [[ $brief =~ ("ahead "([[:digit:]]*)]) ]]; then
    output="$output:A${BASH_REMATCH[2]}"
  fi
  if [[ $brief =~ ("behind "([[:digit:]]*)]) ]]; then
    output="$output:B${BASH_REMATCH[2]}"
  fi
...
function prompt_command() {
  ...
  PS1="\n"
  # python: (env)
  PS1="$PS1$(prompt_python_env)"
  PS1="$PS1$(prompt_battery)"
